...
**Układ widoku treningu:** **pod karuzelą** znajduje się **sticky row** — wiersz UI (przyklejony przy przewijaniu listy) do **dodawania ćwiczeń do bieżącego treningu** (dzisiejszej sesji). Wewnątrz niego jest m.in. **selektor partii ciała** (**body part selector**, np. dropdown), a obok niego wybór **ćwiczenia** (exercise) z katalogu.

**Body part selector (FE, sticky row):** wartości są **zhardcodowane** po stronie klienta — **wyłącznie** te sześć identyfikatorów (API / ścieżka): `shoulders`, `arms`, `chest`, `abs`, `back`, `legs`. Innych opcji w tym selektorze nie ma.

---

## Model treningu i API (twarde reguły)

Te zasady są spójne z regułą Cursor [`.cursor/rules/product-brief.mdc`](.cursor/rules/product-brief.mdc) (agent powinien je tam utrzymywać przy zmianach).
...
| **sticky row**                       | Na **widoku treningu**, **pod karuzelą**: wiersz do **dodawania ćwiczeń** do **bieżącego treningu**; zawiera m.in. **selektor partii ciała** (body part selector) oraz wybór **ćwiczenia**; pozostaje widoczny przy przewijaniu listy (układ „sticky”). W selektorze partii ciała FE ma **tylko** zhardcodowane wartości: `shoulders`, `arms`, `chest`, `abs`, `back`, `legs`.                                                                                                                                                                                        |
...
4. **Sticky row** — **pod karuzelą** na widoku treningu: **selektor partii ciała** (zhardcodowane na FE: `shoulders`, `arms`, `chest`, `abs`, `back`, `legs`) + wybór **ćwiczenia**, **dodawanie** do **bieżącego treningu** (wiersz pozostaje przy przewijaniu listy).
5. **Szablony (template)** — zapisane plany: kolejność ćwiczeń + **waga** i **obciążenie** per zestaw / ćwiczenie; zastosowanie szablonu w bieżącym treningu.
